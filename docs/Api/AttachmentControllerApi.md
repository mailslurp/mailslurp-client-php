...
```php
uploadAttachmentBytes($inline_object1, $content_type, $filename): string[]
```
...
$inline_object1 = new \MailSlurp\Models\InlineObject1(); // \MailSlurp\Models\InlineObject1
...
try {
    $result = $apiInstance->uploadAttachmentBytes($inline_object1, $content_type, $filename);
    print_r($result);
} catch (Exception $e) {
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **inline_object1** | [**\MailSlurp\Models\InlineObject1**](../Model/InlineObject1)|  |
...
```php
uploadMultipartForm($content_type, $filename, $x_filename, $inline_object): string[]
```
