...
```php
createTrackingPixel($create_tracking_pixel_options): \MailSlurp\Models\TrackingPixelDto
```
...
```php
getAllTrackingPixels($page, $size, $sort, $search_filter, $since, $before): \MailSlurp\Models\PageTrackingPixelProjection
```
...
```php
getTrackingPixel($id): \MailSlurp\Models\TrackingPixelDto
```
