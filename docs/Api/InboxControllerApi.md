...
```php
createInbox($email_address, $tags, $name, $description, $use_domain_pool, $favourite, $expires_at, $expires_in, $allow_team_access, $inbox_type, $virtual_inbox): \MailSlurp\Models\InboxDto
```
...
```php
sendWithSchedule($inbox_id, $send_email_options, $send_at_timestamp, $send_at_now_plus_seconds, $validate_before_enqueue)
```
...
try {
    $apiInstance->sendWithSchedule($inbox_id, $send_email_options, $send_at_timestamp, $send_at_now_plus_seconds, $validate_before_enqueue);
} catch (Exception $e) {
