...
```php
doesInboxExist($email_address, $allow_catch_all): \MailSlurp\Models\InboxExistsDto
```
...
$allow_catch_all = True; // bool

try {
    $result = $apiInstance->doesInboxExist($email_address, $allow_catch_all);
    print_r($result);
} catch (Exception $e) {
