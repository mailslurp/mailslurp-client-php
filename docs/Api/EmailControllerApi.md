...
> \MailSlurp\Models\SentEmailDto replyToEmail1($email_id, $reply_to_email_options)
...
try {
    $result = $apiInstance->replyToEmail1($email_id, $reply_to_email_options);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling EmailControllerApi->replyToEmail1: ', $e->getMessage(), PHP_EOL;
}
?>
```
