...
> \MailSlurp\Models\Alias createAlias($create_alias_options)
...
> \MailSlurp\Models\AliasVerificationResult getAliasVerification($alias_id, $email_address, $verification_token)
...
try {
    $result = $apiInstance->getAliasVerification($alias_id, $email_address, $verification_token);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling AliasControllerApi->getAliasVerification: ', $e->getMessage(), PHP_EOL;
}
?>
```
...
[**\MailSlurp\Models\AliasVerificationResult**](../Model/AliasVerificationResult.md)
