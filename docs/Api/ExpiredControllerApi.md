...
> \MailSlurp\Models\PageExpiredInboxRecordProjection getExpiredInboxes($before, $page, $since, $size, $sort)
...
try {
    $result = $apiInstance->getExpiredInboxes($before, $page, $since, $size, $sort);
    print_r($result);
} catch (Exception $e) {
