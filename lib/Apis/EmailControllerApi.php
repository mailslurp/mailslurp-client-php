...
class EmailControllerApi
{
    ...
    protected function getEmailHTMLQueryRequest($email_id, $html_selector)
    {
        ...
        $resourcePath = '/emails/{emailId}/html-query';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        if ($html_selector !== null) {
            $resourcePath = str_replace(
                '{' . 'htmlSelector' . '}',
                ObjectSerializer::toPathValue($html_selector),
                $resourcePath
            );
        }
     ...
     * Parse and return text from an email, stripping HTML and decoding encoded characters
     *
     * @param  bool $decode_html_entities Decode HTML entities (required)
     ...
     * @return \MailSlurp\Models\EmailTextLinesResult
     */
    public function getEmailTextLines($decode_html_entities, $email_id)
    {
        list($response) = $this->getEmailTextLinesWithHttpInfo($decode_html_entities, $email_id);
        return $response;
    }
     ...
     * @return array of \MailSlurp\Models\EmailTextLinesResult, HTTP status code, HTTP response headers (array of strings)
     */
    public function getEmailTextLinesWithHttpInfo($decode_html_entities, $email_id)
    {
        $request = $this->getEmailTextLinesRequest($decode_html_entities, $email_id);
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function getEmailTextLinesAsync($decode_html_entities, $email_id)
    {
        return $this->getEmailTextLinesAsyncWithHttpInfo($decode_html_entities, $email_id)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function getEmailTextLinesAsyncWithHttpInfo($decode_html_entities, $email_id)
    {
        $returnType = '\MailSlurp\Models\EmailTextLinesResult';
        $request = $this->getEmailTextLinesRequest($decode_html_entities, $email_id);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    protected function getEmailTextLinesRequest($decode_html_entities, $email_id)
    {
        // verify the required parameter 'decode_html_entities' is set
        if ($decode_html_entities === null || (is_array($decode_html_entities) && count($decode_html_entities) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $decode_html_entities when calling getEmailTextLines'
            );
        }
        // verify the required parameter 'email_id' is set
        if ($email_id === null || (is_array($email_id) && count($email_id) === 0)) {
        ...
        $resourcePath = '/emails/{emailId}/text';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        if ($decode_html_entities !== null) {
            $resourcePath = str_replace(
                '{' . 'decodeHtmlEntities' . '}',
                ObjectSerializer::toPathValue($decode_html_entities),
                $resourcePath
            );
        }
        // path params
